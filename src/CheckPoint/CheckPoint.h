...
public:
	...
	void SetInfo(eCP_Num Num, VECTOR vPos, VECTOR vSize, float fRad);
	...
	void SetIsArrived(bool IsFrag) { m_IsArrived = IsFrag; }			//到着したフラグ設定

	eCP_Num GetCPNum() { return m_eCP_Num; }							//チェックポイント番号の取得
	void SetCPNum(eCP_Num Num) { m_eCP_Num = Num; }						//チェックポイント番号の設定
